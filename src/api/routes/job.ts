...
export default (app: Router) => {
   ...
   */
  route.post("/job", middlewares.isAuth, async (req, res) => {
  ...
  route.get("/trend", async (req: Request, res: Response) => {
    const userDetails = await jwt_decode(req.header("authorization"));

    const jobServiceInstance = Container.get(JobService);

    const jobRecords = await jobServiceInstance.GetJobByTrend(userDetails);
  ...
  route.get("/reco", async (req: Request, res: Response) => {
    ...
    const jobRecords = await jobServiceInstance.GetJobByReco(userDetails);
